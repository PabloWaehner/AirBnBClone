...
function Account(props) {
    ...
    const token = useSelector(state => state.auth.token, shallowEqual); // shallowEqual --> checks to see if the fields are different (shallow comparison)

    const [pastBookings, setPastBookings] = useState([])
    const [upcomingBookings, setUpcomingBookings] = useState([])

    // useEffect replaces lifecycle methods. The second argument determines when it will run (here it's a [], which is like componentDidMount)
    useEffect(() => {
